...
import { WorldCup } from "../typechain-types/EuropeanCup.sol";
describe("WorldCup", function () {
    ...
    describe("Finalize", function () {
        ...
        it("Should distribute with correct reward", async function () {
            ...
            expect(rewardForA).to.equal(1333333334n)
            expect(rewardForB).to.equal(1333333333n)
            expect(rewardForC).to.equal(1333333333n)
            expect(rewardForD).to.equal(0n)
        })
        console.log(2);
        it("Should emit Finalize Event", async function () {
            ...
            await expect(worldcupIns.finialize(Country.GERMANY))
            .to
            .emit(worldcupIns, "Finialize")
            .withArgs(0,0)
        })
    })
    ...
    describe("ClaimReward", function () {
        ...
        it("Should clear reward after claim", async function () {
            ...
            console.log("balBefore_WCtest",balBefore_WC);
            ...
            expect(balBefore_WC-balAfter_WC).to.equal(rewardForB)

            // lockedAmts记录减少
            expect(balBefore_lockedAmts-balAfter_lockedAmts).to.equal(rewardForB)
        })
    })
});
